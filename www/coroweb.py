...
#将返回一个inspect.Signature类型的对象，值为fn这个函数的所有参数
...
class RequestHandler(object):
	...
	async def __call__(self,request):
			...
			if request.method == 'POST':
				...
				if ct.startswith('application/json'):
					...
					if not isinstance(params,dict):
						return wet.HTTPBadRequest('Json body must be object.')
					kw = params
		...
		if kw is None:
			#match_info主要保存像@get{'/blog/{id}'}里面的id
...
def add_route(app,fn):
	...
	#使用inspect.isgeneratorfunction来判定一个函数是否是一个特殊的 generator 函数
